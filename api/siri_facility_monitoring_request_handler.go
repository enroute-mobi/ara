...
type SIRIFacilityMonitoringRequestHandler struct {
	xmlRequest  *sxml.XMLGetFacilityMonitoring
...
func (handler *SIRIFacilityMonitoringRequestHandler) ConnectorType() string {
	return core.SIRI_FACILITY_MONITORING_REQUEST_BROADCASTER
}

func (handler *SIRIFacilityMonitoringRequestHandler) Respond(params HandlerParams) {
	logger.Log.Debugf("Siri FacilityMonitoring %s", handler.xmlRequest.MessageIdentifier())

	t := clock.DefaultClock().Now()

	response := params.connector.(core.FacilityMonitoringRequestBroadcaster).RequestFacility(handler.xmlRequest, params.message)
	...
	params.message.Type = audit.FACILITY_MONITORING_REQUEST
